...
TEST_CASE("string_table defaults", "[string_table]") {
    ...
    REQUIRE(interned.hash == std::hash<std::string_view>{}({}));

    std::string empty_string{};
    auto &empty2 = string_table.intern(empty_string);
    REQUIRE(&empty2 == &interned);

    const char empty_cstr[] = "";
    auto &empty3 = string_table.intern({empty_cstr, 0});
...
TEST_CASE("string_table basics", "[string_table]") {
    ...
    interned_string &interned = string_table.intern(datadog);

    REQUIRE(interned.size == datadog.size());
    REQUIRE(interned.offset == 1);  // remember, there is an empty string at 0
    REQUIRE(interned.hash == std::hash<std::string>{}(datadog));

    interned_string &again = string_table.intern(datadog);
...
TEST_CASE("intern a few strings", "[string_table]") {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        interned_string &interned = string_table.intern(input);

        REQUIRE(interned.size == input.size());

        // remember, there is an empty string at 0
        REQUIRE(interned.offset == i + 1);
        REQUIRE(interned.hash == std::hash<std::string>{}(input));
    }
